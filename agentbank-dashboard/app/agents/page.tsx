...
import { getAgents, createAgent, freezeAgent, Agent } from '@/lib/api'
import { Plus, Lock, Unlock, PauseCircle, Zap, RefreshCw, Copy, Settings } from 'lucide-react'
      ...
      <div style={{ padding: 20 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            <div className={`status-dot ${agent.status}`} style={{ marginTop: 2 }} />
            <div>
              <div style={{ fontWeight: 500, fontSize: 14 }}>{agent.name}</div>
              <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 1 }}>{agent.description || 'No description'}</div>
            </div>
          </div>
